...
GRANT SELECT (id, full_name, bio, avatar_url, nationality, motorcycle, created_at, notifications_seen_at, notifications_cleared_at) ON profiles TO authenticated;
GRANT INSERT (id, full_name, bio, avatar_url, push_token, nationality, motorcycle) ON profiles TO authenticated;
GRANT UPDATE (full_name, bio, avatar_url, push_token, nationality, motorcycle, notifications_seen_at, notifications_cleared_at) ON profiles TO authenticated;
...
GRANT SELECT ON request_notification_events TO authenticated;
CREATE POLICY rne_participant_select ON public.request_notification_events
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.stay_requests sr
    WHERE sr.id = request_id AND (sr.guest_id = auth.uid() OR sr.host_id = auth.uid())
  ));
...
-- ── Feedback to the developers (capture + email; no tipping yet) ──
...
CREATE POLICY "feedback_insert" ON public.feedback FOR INSERT TO public WITH CHECK (auth.uid() = user_id);
...
BEGIN
  ...
  SELECT user_id, photos INTO v_owner, v_photos FROM host_locations WHERE id = p_id;
...
$function$
;
REVOKE EXECUTE ON FUNCTION public.delete_host_location(uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.delete_host_location(uuid) TO authenticated;
...
-- Additive channel alongside native Expo push (profiles.push_token). RLS owner-only; edge
-- functions read via service_role. Mirrors migration 00000000000009.
